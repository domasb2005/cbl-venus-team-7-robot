...
void follow_line() {
    ...
    while (running) {
        ...
        if (raw_r2 > raw_r1 && raw_r2 > raw_r3) {
            ...
            int sweep_size = 1;   // Base multiplier for sweeping
            int sweep_direction = -1;  // Start with left (-1), then alternate
        
            while (adc_read_channel(IR_2R) < THRESHOLD && running) {
                printf("[i] Sweeping (size: %d, direction: %s)...\n", sweep_size * 100, sweep_direction < 0 ? "LEFT" : "RIGHT");
        
                for (int i = 0; i < sweep_size; i++) {
                    // Check for crater before each step
                    double v_l1 = adc_read_channel(IR_1L);
                    double v_l2 = adc_read_channel(IR_2L);
                    double v_l3 = adc_read_channel(IR_3L);
                    
                    if (v_l1 > LEFT_THRESHOLD || v_l2 > LEFT_THRESHOLD || v_l3 > LEFT_THRESHOLD) {
                        printf("[!] Crossing detected during sweep. Switching to crater avoidance...\n");
                        avoid_crater();
                        goto end_sweep;
                    }
                    ...
                    // Break if R2 is now on black
                    if (adc_read_channel(IR_2R) >= THRESHOLD) {
                        printf("[✓] R2 detected black during sweep. Resuming line following.\n");
                        goto end_sweep;
                    }
                }
        
                // Alternate direction, increase sweep size
                sweep_direction *= -1;
                sweep_size++;
            }
        
        end_sweep:
            ; // null statement to terminate goto target
        } else {
            sleep_msec(50);
        }
